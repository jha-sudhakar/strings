...
void key_index_sort::sort_using_key_index()
{
	...
	for(int pos=len-1; pos>-1; pos--)
	{
		...
		{
			char index = arr_vp[i]->at(pos);
			//cout <<"Pos= " << pos <<", at index " << index <<" char = "<< arr_vp[i]->at(pos) << endl;
			count_v[1+index]++;
		}	
		...
		{
			buf[count_v[arr_vp[i]->at(pos)]] = arr_vp[i];
			 count_v[arr_vp[i]->at(pos)]++;
		}
